...
class LobbyScene extends Scene {
    ...
    create() {
        ...
        localRooms.map((room, i) => {
            this.add.text(100, 100 + (i*20), room).setInteractive().on('pointerover', (btn) => btn.add.setStyle({ fill: '#006db2' })).on('pointerout', (btn) => btn.setStyle({ fill: '#fff' })).on('pointerdown', () => this.scene.start('GameScene'));
